...
class monthly_cashflow_xls_parser(report_sxw.rml_parse):

	def __init__(self, cr, uid, name, context):
		super(monthly_cashflow_xls_parser, self).__init__(cr, uid, name,
														 context=context)
	...
	def get_cashflow(self,data,objects):
		...
		query="""select coalesce(receivable.code,payment.code) as code,coalesce(receivable.credit,0.00) as credit,coalesce(payment.debit,0.00) as debit
			from (select
			aml.period_id,ap.code,sum(coalesce(aml.credit,0.00)) as credit
			...
			group by aml.period_id,ap.code
			) receivable
			full outer join (
			select aml2.period_id,ap2.code,sum(coalesce(aml2.debit,0.00)) as debit
			from account_move_line aml2
			left join account_account aa2 on aml2.account_id=aa2.id
			left join account_period ap2 on aml2.period_id=ap2.id
			...
			group by aml2.period_id,ap2.code
			) payment
			on receivable.period_id=payment.period_id
...
class monthly_cashflow_xls(report_xls):

	def __init__(self, name, table, rml=False, parser=False, header=True,
				 store=False):
		super(monthly_cashflow_xls, self).__init__(
	...
	def generate_xls_report(self, _p, _xs, data, objects, wb):
		...
		if data['t_report']=='monthly_cashflow':
			ws = wb.add_sheet("Monthly Cashflow")
			...
			ws.write_merge(0,0,0,4,"Laporan Cashflow per Bulan",title_style_center)
			...
			ws.write(9,0,"Keterangan Penerimaan",normal_bold_style_a)
			...
			for cash in cashflow:
				ws.write(row_pos,0,cash['code'],normal_style)
				ws.write(row_pos,1,cash['credit'],normal_style_float)
				ws.write(row_pos,2,cash['code'],normal_style_float)
				ws.write(row_pos,3,cash['debit'],normal_style_float)
				ws.write(row_pos,4,cash['credit'] or 0.0-cash['debit'] or 0.0,normal_style_float)
				SUBTOTAL_C +=cash['credit']
				SUBTOTAL_D +=cash['debit']
				row_pos+=1
...
monthly_cashflow_xls('report.monthly.cashflow.report.xls', 'account.move.line',
					parser=monthly_cashflow_xls_parser)
