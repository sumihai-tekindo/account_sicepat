...
class master_analytic_account(models.Model):
    _name = "master.analytic.account"
    _description = "Master Link Analytic Account"
    _order = "pruduct_id"
    _rec_name = "account_analytic_id"

    pruduct_id = fields.Many2one('product.product',string='Product')
    departmen_id = fields.Many2one('account.invoice.department',string='Department')
    account_analytic_id = fields.Many2one('account.analytic.account', string="Account Analytic")



class account_invoice_line_inherit1(models.Model):
