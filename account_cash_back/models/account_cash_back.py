		...
		if context.get('start_date',False) and context.get('end_date',False):
			...
			query_cashback = """select 
							...
							left join (
								select ail.partner_id,
									round(sum(case when ai.type='out_invoice' then (100.00/(100.00-rp.current_discount))*(ail.price_unit)*ail.quantity 
			...
			for line in self.browse(cr,uid,ids,context=context):
				...
				query_cashback = """select 
								...
								left join (
									select ail.partner_id,
									round(sum(case when ai.type='out_invoice' then (100.00/(100.00-rp.current_discount))*(ail.price_unit)*ail.quantity else (100.00/(100.00-rp.current_discount))*(-1*ail.price_unit)*ail.quantity end),2) as omzet_before_disc,
									sum(case when ai.type='out_invoice' then ail.price_subtotal else -1*ail.price_subtotal end) as after_disc
									from account_invoice_line ail 
									left join account_invoice ai on ail.invoice_id=ai.id
									left join res_partner rp on ai.partner_id=rp.id
