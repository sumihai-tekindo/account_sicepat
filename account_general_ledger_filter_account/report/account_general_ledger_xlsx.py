...
class ReportGeneralLedgerXlsx(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        for obj in objs:
            ...
            worksheet.set_column('W:W', 7.86)
            worksheet.set_column('X:X', 7.86)
            worksheet.set_column('Y:Y', 7.86)
            worksheet.set_column('Z:Z', 7.86)
            worksheet.set_column('AA:AA', 7.86)
            worksheet.set_column('AB:AB', 7.86)
            worksheet.set_column('AC:AC', 7.86)
            ...
            worksheet.merge_range(0, 0, 0, 28, 'General ledger', cell_format_title)
            worksheet.merge_range(1, 0, 1, 28, None, cell_format_title)
            worksheet.merge_range(2, 0, 2, 6, 'Chart of Accounts:', cell_format_bold)
            worksheet.merge_range(2, 7, 2, 13, 'Fiscal Year:', cell_format_bold)
            worksheet.merge_range(2, 14, 2, 21, 'Journals:', cell_format_bold)
            worksheet.merge_range(2, 22, 2, 28, 'Display Account', cell_format_bold)
            worksheet.merge_range(3, 0, 3, 6, _p.get_account(data), cell_format)
            worksheet.merge_range(3, 7, 3, 13, _p.get_fiscalyear(data), cell_format)
            worksheet.merge_range(3, 14, 3, 21, ', '.join([ lt or '' for lt in _p.get_journal(data) ]), cell_format_wrap)
            display_account = ''
            ...
            worksheet.merge_range(3, 22, 3, 28, display_account, cell_format)
            worksheet.merge_range(4, 0, 4, 28, None, cell_format)
            
            worksheet.merge_range(5, 0, 5, 6, 'Filter By:', cell_format_bold)
            worksheet.merge_range(5, 7, 5, 13, 'Sorted By:', cell_format_bold)
            worksheet.merge_range(5, 14, 5, 21, 'Target Moves:', cell_format_bold)
            filter_by = ''
            ...
            worksheet.merge_range(6, 0, 6, 6, filter_by, cell_format_wrap)
            worksheet.merge_range(6, 7, 6, 13, _p.get_sortby(data), cell_format)
            worksheet.merge_range(6, 14, 6, 21, _p.get_target_move(data), cell_format)
            worksheet.merge_range(7, 0, 7, 28, None, cell_format)
            ...
            worksheet.merge_range(8, 19, 8, 20, 'Department', cell_format_bold)
            worksheet.merge_range(8, 21, 8, 22, 'Debit', cell_format_bold)
            worksheet.merge_range(8, 23, 8, 24, 'Credit', cell_format_bold)
            worksheet.merge_range(8, 25, 8, 26, 'Balance', cell_format_bold)
            if data['form']['amount_currency']:
                worksheet.merge_range(8, 27, 8, 28, 'Currency', cell_format_bold)
            row = 9
            for childrenaccount in _p.get_children_accounts(obj):
                cell_format_bold_indent.set_indent(childrenaccount['level'] - 1)
                worksheet.merge_range(row, 0, row, 14, childrenaccount['code'] + ' ' + childrenaccount['name'], cell_format_bold_indent)
                worksheet.merge_range(row, 21, row, 22, _p.sum_debit_account(childrenaccount), cell_format_decimal_bold)
                worksheet.merge_range(row, 23, row, 24, _p.sum_credit_account(childrenaccount), cell_format_decimal_bold)
                worksheet.merge_range(row, 25, row, 26, _p.sum_balance_account(childrenaccount), cell_format_decimal_bold)
                if data['form']['amount_currency']:
                    worksheet.merge_range(row, 27, row, 28, _p.sum_currency_amount_account(childrenaccount), cell_format_decimal)
                row += 1
                for line in _p.lines(childrenaccount):
                    ...
                    worksheet.merge_range(row, 19, row, 20, line['department'], cell_format)
                    worksheet.merge_range(row, 21, row, 22, line['debit'], cell_format_decimal)
                    worksheet.merge_range(row, 23, row, 24, line['credit'], cell_format_decimal)
                    worksheet.merge_range(row, 25, row, 26, line['progress'], cell_format_decimal)
                    if data['form']['amount_currency']:
                        worksheet.merge_range(row, 27, row, 28, line['amount_currency'] > 0.0 and line['amount_currency'] or '', cell_format_decimal)
                    row += 1
